...
for epoch in range(10):
    for ii, (img, bbox_, label_, scale) in tqdm(enumerate(dataloader)):
        ...
        rpn_loc_loss, rpn_cls_loss, roi_loc_loss, roi_cls_loss, total_loss= Faster_RCNN.train_batch(img, bbox, label, scale)
...
with open("rpn_loc_loss.txt", "w") as f:
    for s in rpn_loc_loss_log:
        f.write(str(s) + '\n')

with open("rpn_cls_loss_log.txt", "w") as f:
    for s in rpn_cls_loss_log:
        f.write(str(s) + '\n')

with open("roi_loc_loss_log.txt", "w") as f:
    for s in roi_loc_loss_log:
        f.write(str(s) + '\n')

with open("roi_cls_loss_log.txt", "w") as f:
    for s in roi_cls_loss_log:
        f.write(str(s) + '\n')

with open("total_loss_log.txt", "w") as f:
